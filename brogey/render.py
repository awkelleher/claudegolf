...
async function loadFromSupabase() {
  ...
  while (true) {
    const { data, error } = await sb
      .from('shots')
      .select('shot_num,club,club_speed_mps,attack_angle_deg,ball_speed_mps,spin_rate_rpm,carry_m,side_m,smash_factor,total_m,launch_angle_deg,max_height_m,landing_angle_deg,hang_time_s,curve_m,raw_measurement,session_id,sessions(session_date,source)')
    ...
    data.forEach(row => {
      ...
      RAW.push({
        ...
        source: row.sessions ? row.sessions.source : null,
        shot_num: row.shot_num,
        club: row.club,
        // Always-meaningful primary metrics
        ...
        // Cross-source first-class flight metrics (now real columns)
        total_m: row.total_m,
        launch_angle_deg: row.launch_angle_deg,
        max_height_m: row.max_height_m,
        landing_angle_deg: row.landing_angle_deg,
        hang_time_s: row.hang_time_s,
        curve_m: row.curve_m,
        // TrackMan-only diagnostics still pulled from raw_measurement
